...
from .layers import LoRALayer, PlainMultiheadAttentionLoRA, LinearAttentionLoRA
...
def apply_lora_clip(args, clip_model):
    list_lora_layers = []
...
def save_lora_clip(args, list_lora_layers):
    weights = {}
    ...
    torch.save(save_data, args.lora_save_path)


def load_lora_clip(args, list_lora_layers):
    # to manage names like ViT-B/16
    load_path = args.lora_save_path
...
def apply_lora_conch(args, conch_model):
    assert args.position == 'all', 'support only all for now'
    list_lora_layers = []
    if args.encoder == 'text' or args.encoder == 'both':
        text_encoder = conch_model.text.transformer
        for block in text_encoder.resblocks:
            for name, submodule in block.named_children():
                if isinstance(submodule, nn.MultiheadAttention):
                    new_multi_head_lora = PlainMultiheadAttentionLoRA(
                        submodule, enable_lora=args.params, r=args.r, lora_alpha=args.alpha, dropout_rate=args.dropout_rate)
    ...
    if args.encoder == 'vision' or args.encoder == 'both':
        vision_encoder = conch_model.visual.trunk
        for block in vision_encoder.blocks:
            for name, submodule in block.named_children():
                if isinstance(submodule, Attention):
                    new_attention_lora = LinearAttentionLoRA(
                        submodule, enable_lora=args.params, r=args.r, lora_alpha=args.alpha, dropout_rate=args.dropout_rate
                    )
...
def save_lora_conch(args, list_lora_layers):
    weights = {}
    for i, layer in enumerate(list_lora_layers):
        layer_weights = {}
        ...
        elif isinstance(layer, LinearAttentionLoRA):
            if set(args.params).issubset(['q', 'k', 'v']):
                layer_weights['qkv'] = {
                    'w_lora_A': layer.qkv.w_lora_A.data,
                    'w_lora_B': layer.qkv.w_lora_B.data
                }
            if 'o' in args.params:
                layer_weights['proj'] = {
                    'w_lora_A': layer.proj.w_lora_A.data,
...
def load_lora_conch(args, list_lora_layers):
    load_path = args.lora_save_path
    ...
    for i, layer in enumerate(list_lora_layers):
        ...
        if isinstance(layer, PlainMultiheadAttentionLoRA):
            ...
            if set(args.params).issubset(['q', 'k', 'v']):
                layer.qkv.w_lora_A.data.copy_(layer_weights['qkv']['w_lora_A'])
                layer.qkv.w_lora_B.data.copy_(layer_weights['qkv']['w_lora_B'])
            if 'o' in args.params and 'proj' in layer_weights:
                layer.proj.w_lora_A.data.copy_(layer_weights['proj']['w_lora_A'])
                layer.proj.w_lora_B.data.copy_(layer_weights['proj']['w_lora_B'])
...
def apply_lora(args, model):
    if args.vl_model == "conch":
        return apply_lora_conch(args, model)
    elif args.vl_model == 'quiltNet':
        return apply_lora_clip(args, model)
    else:
        raise NotImplementedError
...
def save_lora(args, list_lora_layers):
    if args.vl_model == "conch":
        return save_lora_conch(args, list_lora_layers)
    elif args.vl_model == 'quiltNet':
        return save_lora_clip(args, list_lora_layers)
    else:
        raise NotImplementedError
...
def load_lora(args, list_lora_layers):
    ...
    if args.vl_model == "conch":
        load_lora_conch(args, list_lora_layers)
    elif args.vl_model == 'quiltNet':
        load_lora_clip(args, list_lora_layers)
    else:
        raise NotImplementedError
