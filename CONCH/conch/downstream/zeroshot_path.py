...
#             classname_embeddings = model.encode_text(token_ids, normalize=True)
...
@torch.no_grad()
def zero_shot_classifier(vl_model, model, classnames, templates, tokenizer, device):
    """
    classnames: list of lists of classnames (one list of classnames per class)
    templates: list of templates 
    """
...
def run_zeroshot(model, classifier, dataloader, device, dump_results=False, 
    ...
    for batch_idx, data in enumerate(tqdm(dataloader)): 
        ...
        image_features = model.encode_image(imgs, normalize=True)
