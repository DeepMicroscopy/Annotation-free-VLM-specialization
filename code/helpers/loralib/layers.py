...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        self, 
        existing_mergedLinear: nn.Linear,
        r: int = 0, 
        lora_alpha: int = 1, 
        ...
        **kwargs
        ...
        in_features = existing_mergedLinear.in_features
        out_features = existing_mergedLinear.out_features
        assert out_features % len(enable_lora) == 0, 'The length of enable_lora must divide out_features'
        self.enable_lora = enable_lora

        nn.Linear.__init__(self, in_features, out_features, **kwargs)
        self.load_state_dict(existing_mergedLinear.state_dict())
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha)
        ...
        if r > 0 and any(enable_lora):
            ...
            self.w_lora_B = nn.Parameter(self.weight.new_zeros((out_features // len(enable_lora) * sum(enable_lora), r))) # weights for Conv1D with groups=sum(enable_lora)
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
            # Compute the indices
    ...
    def merge_BA(self, param_name: str):
        lora_name = self.params_with_lora[param_name]
        delta_w = F.conv1d(eval(f'self.{lora_name}_lora_A').unsqueeze(0), eval(f'self.{lora_name}_lora_B').unsqueeze(-1), groups=sum(self.enable_lora)).squeeze(0)
...
class LinearAttentionLoRA(nn.Module):
    def __init__(self, 
                 existing_attn: nn.Module,
                 enable_lora: list=['q', 'k', 'v', 'o'],
                 r: int = 0,
                 lora_alpha: int = 1,
                 dropout_rate: float = 0.,
        ...
        --> replace qkv with a MergedLinear layer and proj with a LinearLoRA layer

        """
        super().__init__()
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, dropout_rate=dropout_rate)
        enable_lora = [
            True if 'q' in enable_lora else False,
            True if 'k' in enable_lora else False,
            True if 'v' in enable_lora else False
            ]
        self.num_heads = existing_attn.num_heads
        self.head_dim = existing_attn.head_dim
        self.scale = existing_attn.scale
        self.fused_attn = existing_attn.fused_attn

        self.qkv = MergedLinear(existing_mergedLinear=existing_attn.qkv, 
                                enable_lora=enable_lora,
                                r=r, lora_alpha=lora_alpha, dropout_rate=dropout_rate)
        self.attn_drop = nn.Dropout(existing_attn.attn_drop.p)
        self.proj = LinearLoRA(existing_linear=existing_attn.proj,
                               r=r, lora_alpha=lora_alpha, dropout_rate=dropout_rate)
        self.proj_drop = nn.Dropout(existing_attn.proj_drop.p)
